...
const MetricRow = ({ label, value }) => (
  <div className="flex justify-between items-center py-3 border-b border-gray-50 dark:border-slate-800 last:border-0 hover:bg-gray-50/30 dark:hover:bg-slate-800/30 px-2 rounded-lg transition-colors">
    ...
    <div className="flex items-center gap-4">
      ...
      <span className="font-bold text-gray-900 dark:text-white min-w-[3.5rem] text-right text-sm">{value.toFixed(1)}/100</span>
    </div>
  </div>
);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            {score !== undefined && (
              <span className={`px-3 py-0.5 rounded-full text-xs font-medium border ${score >= 80 ? 'bg-green-50 text-green-700 border-green-100' :
            ...
            {verdict && (
              <span className={`px-3 py-0.5 rounded-full text-xs font-medium border ${verdict.includes('HUMAN') || verdict.includes('CLEAN') || verdict.includes('ORIGINAL') ? 'bg-green-50 dark:bg-emerald-900/20 text-green-700 dark:text-emerald-400 border-green-100 dark:border-emerald-500/20' :
...
const EvaluationResults = () => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center h-[70vh] animate-pulse">
        ...
        <p className="mt-8 text-gray-500 dark:text-slate-400 font-medium text-xs">Synthesizing AI Engine Outputs...</p>
      </div>
    );
  }
  ...
  if (error || !evaluation) {
    return (
      <div className="max-w-2xl mx-auto mt-20 p-12 bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-red-50 dark:border-red-900/10 text-center animate-fade-in">
        ...
        <h2 className="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Evaluation Unreachable</h2>
        ...
        <button
          ...
          className="mt-8 px-8 py-3 bg-gray-900 dark:bg-indigo-600 text-white rounded-2xl font-bold text-sm uppercase tracking-widest hover:bg-black dark:hover:bg-indigo-700 transition-all shadow-xl shadow-gray-200 dark:shadow-none"
        >
          Return to Console
        </button>
      </div>
  ...
  // AI Feedback Extrapolations
  const feedResult = evaluation?.structured_feedback || {};
  const execSummary = feedResult.executive_summary?.overall_assessment || evaluation.ai_feedback || "Synthesizing descriptive feedback...";
  const strengths = feedResult.strengths || [];
  const improvements = feedResult.areas_for_improvement || [];
  const actions = feedResult.actionable_recommendations || [];
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-10 space-y-10 animate-fade-in mb-24">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-gray-100 dark:border-slate-800 pb-8">
        <div className="space-y-4">
          <button
            onClick={() => navigate(-1)}
            className="group flex items-center text-xs font-bold text-gray-400 uppercase tracking-[0.2em] hover:text-blue-600 transition-colors"
          >
            <ArrowLeftIcon className="w-4 h-4 mr-2 group-hover:-translate-x-1 transition-transform" />
            Back to Registry
          </button>
          <div>
            <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 dark:text-white tracking-tight leading-tight">{evaluation.project?.title}</h1>
        ...
        <div className="flex gap-3">
          {evaluation.is_finalized ? (
            <div className="flex items-center gap-3 px-6 py-4 bg-indigo-600 rounded-3xl text-white shadow-xl shadow-indigo-200 dark:shadow-none">
              ...
              <span className="font-bold text-xs uppercase tracking-widest">Faculty Verified</span>
            </div>
          ) : (
            <div className="flex items-center gap-3 px-6 py-4 bg-white dark:bg-slate-900 border-2 border-yellow-100 dark:border-amber-900/30 rounded-3xl text-yellow-600 dark:text-amber-400 shadow-xl shadow-yellow-50 dark:shadow-none">
              ...
              <span className="font-bold text-xs uppercase tracking-widest">AI Raw Inference</span>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 bg-gray-950 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden flex flex-col justify-between min-h-[400px]">
          <div className="relative z-10 flex justify-between items-start">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.3em] text-gray-500 mb-6">Aggregate Performance Index</p>
              <div className="flex items-baseline gap-4">
                <span className="text-[10rem] font-bold leading-none tracking-tighter text-blue-500">{Math.round(totalScore)}</span>
            ...
            <div className="bg-white/5 backdrop-blur-3xl rounded-[2rem] p-8 border border-white/10 flex flex-col items-center justify-center min-w-[140px]">
              <span className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">Grade</span>
              <span className="text-6xl font-bold text-white">{stats.grade}</span>
            </div>
          </div>

          <div className="relative z-10 grid grid-cols-2 xs:grid-cols-4 gap-8 pt-10 border-t border-white/5">
            <div>
              <p className="text-xs font-medium text-slate-600">Interpretation</p>
              <p className="text-lg font-bold mt-1 text-gray-300">{stats.interpretation}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-slate-600">Global Rank</p>
              <p className="text-lg font-bold mt-1 text-blue-400">{stats.rank}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-slate-600">Integrity</p>
          ...
          {[
            ...
            <div key={i} className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-3xl p-6 shadow-sm flex items-center justify-between group hover:border-blue-200 dark:hover:border-indigo-500 transition-all">
              ...
              <div className="text-right">
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{Math.round(m.score || 0)}%</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl shadow-gray-100 dark:shadow-none border border-gray-50 dark:border-slate-800 p-10 flex flex-col">
          <div className="mb-10">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Logical Footprint</h2>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl shadow-gray-100 dark:shadow-none border border-gray-50 dark:border-slate-800 p-10 flex flex-col">
          <div className="mb-10">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Weighted Contributions</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-2">
          <h3 className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-[0.3em] mb-6 px-4">Engine Granularity</h3>
          ...
          >
            ...
            <Link to={`/evaluations/${id}/code-analyzer`} className="mt-4 flex items-center gap-2 text-xs font-bold text-blue-600 dark:text-indigo-400 hover:underline uppercase tracking-widest">
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
              <div className="p-6 bg-gray-50 dark:bg-slate-800/50 rounded-3xl text-center">
                <p className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Inference Probability</p>
                <p className="text-3xl font-bold text-gray-900 dark:text-white">{(evaluation.ai_detection_result?.ai_generated_probability * 100 || 0).toFixed(1)}%</p>
              </div>
              <div className="p-6 bg-gray-50 dark:bg-slate-800/50 rounded-3xl text-center">
                <p className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Detect Model Confidence</p>
                <p className="text-3xl font-bold text-gray-900 dark:text-white">97.4%</p>
              </div>
            </div>
            <p className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-3 px-2">Identified Signatures</p>
            ...
            <Link to={`/evaluations/${id}/code-detector`} className="mt-4 flex items-center gap-2 text-xs font-bold text-blue-600 dark:text-indigo-400 hover:underline uppercase tracking-widest">
          ...
          >
            <div className="space-y-6">
              <div className="flex items-center justify-between p-6 bg-gray-50 dark:bg-slate-800/50 rounded-3xl">
                <div>
                  <p className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Cross-Submission Similarity</p>
                  <span className="text-2xl font-bold text-gray-900 dark:text-white">{evaluation.plagiarism_result?.max_similarity_percent || 0}%</span>
                </div>
                <div className="text-right">
                  <p className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Logic Overlap</p>
                  <span className={`text-2xl font-bold ${evaluation.plagiarism_detected ? 'text-rose-500' : 'text-emerald-500'}`}>
              ...
              {evaluation.plagiarism_result?.similar_sections?.length > 0 && (
                <div className="p-5 border border-rose-100 dark:border-rose-900/30 bg-rose-50/30 dark:bg-rose-900/10 rounded-2xl">
                  <p className="text-xs font-semibold text-rose-700 dark:text-rose-400 mb-3">Matching Logic Blocks</p>
              ...
              <Link to={`/evaluations/${id}/plagiarism`} className="flex items-center gap-2 text-xs font-bold text-blue-600 dark:text-indigo-400 hover:underline uppercase tracking-widest">
          ...
          >
            <div className="space-y-6">
              <div className="bg-indigo-50 dark:bg-indigo-900/20 p-6 rounded-3xl border border-indigo-100 dark:border-indigo-500/20">
                <p className="text-xs font-semibold text-indigo-700 dark:text-indigo-400 mb-3">Engine Synthesis</p>
              ...
              {evaluation.alignment_result?.missing_features?.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-rose-500 px-2 mb-3">Documented but Missing in Source</p>
              ...
              <Link to={`/evaluations/${id}/report-aligner`} className="flex items-center gap-2 text-xs font-bold text-blue-600 dark:text-indigo-400 hover:underline uppercase tracking-widest">
        ...
        <div className="space-y-10">
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl shadow-gray-100 dark:shadow-none border border-gray-50 dark:border-slate-800 p-10">
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight mb-8">AI Narrative</h3>
            ...
            <div className="space-y-8">
              {/* Strengths */}
              <div>
                <h4 className="text-[10px] font-bold text-indigo-400 uppercase tracking-[0.2em] mb-4">Core Strengths</h4>
                <ul className="space-y-3">
                  ...
                  )) : <li className="text-sm font-medium text-gray-500 italic p-3">No core strengths identified.</li>}
                </ul>
              </div>

              {/* Improvements with Checkboxes */}
              <div>
                <h4 className="text-[10px] font-bold text-blue-400 uppercase tracking-[0.2em] mb-4">Optimization Required</h4>
                <div className="space-y-3">
                  ...
                  )) : <div className="text-sm font-medium text-gray-500 italic p-4">No optimizations required.</div>}
                </div>
              </div>

              {/* Action Items with Priority Badges */}
              <div>
                <h4 className="text-xs font-semibold text-rose-500 tracking-[0.2em] mb-4">Immediate Actions</h4>
                <div className="space-y-3">
                  {actions.length > 0 ? actions.map((item, i) => {
                    const pri = item.priority || 'MEDIUM';
                    const color = item.color || getPriorityColor(pri);
                    return (
                      <div key={i} className="flex items-center gap-3 p-4 bg-gray-50 dark:bg-slate-800/50 rounded-2xl group cursor-default">
                        <div className={`px-2.5 py-1 rounded-lg bg-${color}-600 text-white text-[8px] font-bold`}>{pri}</div>
                        <span className="text-xs font-bold text-gray-800 dark:text-slate-300">{item.action || item.act}</span>
                      </div>
                    );
                  }) : <div className="text-sm font-medium text-gray-500 italic p-4">No immediate actions required.</div>}
                </div>
              </div>
            </div>
          </div>
          ...
          {evaluation.is_finalized && (
            <div className="bg-blue-600 rounded-[2.5rem] p-10 text-white shadow-2xl shadow-blue-200 relative overflow-hidden">
              ...
              <div className="text-xs font-medium text-blue-200 relative z-10">
