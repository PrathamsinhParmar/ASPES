      ...
      <div className="w-24 sm:w-40 bg-gray-100 dark:bg-slate-800 rounded-full h-1.5 hidden xs:block overflow-hidden">
        <div 
          className={`h-1.5 rounded-full transition-all duration-1000 ease-out ${
            value >= 90 ? 'bg-indigo-500' : value >= 80 ? 'bg-green-500' : value >= 60 ? 'bg-blue-500' : value >= 40 ? 'bg-yellow-500' : 'bg-red-500'
          }`}
          style={{ width: `${value}%` }}
        ></div>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            {verdict && (
              <span className={`px-3 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                verdict.includes('HUMAN') || verdict.includes('CLEAN') || verdict.includes('ORIGINAL') ? 'bg-green-50 dark:bg-emerald-900/20 text-green-700 dark:text-emerald-400 border-green-100 dark:border-emerald-500/20' :
