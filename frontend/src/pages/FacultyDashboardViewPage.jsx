...
const FacultyDashboardViewPage = () => {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-screen bg-transparent dark:bg-slate-950">
  ...
  if (error || !faculty) {
    return (
      <div className="p-8 flex flex-col items-center justify-center gap-4 text-center min-h-screen bg-transparent dark:bg-slate-950">
              ...
              <thead className="bg-gray-50/50 dark:bg-slate-800/50">
                <tr>
                  {['Count', 'Project', 'Language', 'Team', 'Date', 'Status', 'AI Score'].map((h) => (
                    <th
                      key={h}
                      className="px-4 py-4 text-left text-xs font-medium text-slate-500 dark:text-slate-400 whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
